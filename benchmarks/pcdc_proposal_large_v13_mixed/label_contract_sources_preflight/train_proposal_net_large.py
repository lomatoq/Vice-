...
from .proposal_data_contract import (
    TYPED_STRUCTURE_FAMILIES, split_group as contract_split_group,
    typed_macro_families,
    uses_explicit_owner_labels,
)
from .proposal_mixed_corpus import validate_mixed_corpus
from .proposal_filter_cache import validate_filter_cache
...
LABEL_CONTRACT_VERSION = "pcdc-explicit-owner-typed-mixed-replay-labels/v2"
...
def _balanced_structure_family_shares(
    ...
    for row, value in zip(rows, weights):
        ...
        mass[typed[0]] += amount; total += amount
    if total <= 0.0:
        return {}
...
def train(args: argparse.Namespace) -> dict:
    ...
    pair_manifest = args.pair_root / "pairs.jsonl"
    ...
    data_contract_digest = hashlib.sha256(pair_manifest.read_bytes())
    if mixed_report is not None:
        data_contract_digest.update(mixed_report_path.read_bytes())
    corpus_data_contract_sha256 = data_contract_digest.hexdigest()
    ...
    report = {
        ...
        "label_contract": {
            ...
            "typed_structure_factory": (
                "typed-generator/v1 exact family supervision for stroke/"
                "appearance/layer/repeat"
            ),
