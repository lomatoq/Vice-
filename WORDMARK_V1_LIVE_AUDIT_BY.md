...
106. Materialization v2 пабудавана цалкам па знешнім плане (M2-01…M2-10,
    2026-07-26). Фінальная вектарная праграма стала першакласным
    аб'ектам: кантракт+кананічны дайджэст, faithful-праграма з
    байтавым парытэтам рэндэру, легасі-фітэр перанесены ДА суда,
    ownership≠evaluation дамен у судзе, адпаведнасць кампанентаў +
    карыдоры раздзялення 1×/2×/4×, субпіксельная мяжа (marching
    squares), fairness-матэматыка, лексікаграфічны DP
    line/arc/cubic/faithful, appearance-completeness, гонка
    матэрыялізацый, рэфайн рэальных параметраў з замарожанай
    структурай, экспарцёр аддае фрагмент суда, схема справаздачы v3.
    Гейты: 106 юніт-тэстаў + 37 рэгрэсійных зялёныя. Сінтэтычны
    кантроль пацвярджае мэту плана §0: круг 3 дугі, літара O 10
    спанаў, закруглены прамавугольнік і дзве паласы выйграюць
    фізічную нічыю па fairness (8/14 спанаў супраць 160/288), а
    зубчастая форма застаецца піксельна-сумленнай (fair прайграе
    фізічна). Знойдзена і выпраўлена ПЯЦЬ рэальных дэфектаў:
    паўпіксельная канвенцыя (fair-праграмы дастаўляліся на паўпікселя
    ўверх-улева), фрагментацыя кантураў у marching squares (IoU 0.03),
    нулявы «кошт нечэснасці» лесвіцы (вымяраць трэба паварот на
    адзінку даўжыні), немагчымасць сціснуць дугу пры фіксаваных
    канцах (патрэбна зменная падобнасці), і нязгода класіфікацыі
    роляў з правілам валідатара на шчыльных wordmark'ах. Метадалагічны
    вывад: судзіць матэрыялізатар супраць БІНАРНАЙ маскі несапраўдна —
    піксельная копія тады аптымальная па азначэнні; аракул пераведзены
    на пакрыццё з крыніцы. Дастаўленага доказу ПАКУЛЬ НЯМА: першы
    прагон з флагам ішоў са старым дэфектам ролей (fair кідаў
    выключэнне → сумленны fall-open, 100/100 ідэнтычна H11);
    перазапуск ідзе. Латэнтнасць не ў бюджэце §12 (fair DP p95 334 мс
    на кантур; гонка 7 с p95 на лоцы) — план §10 адкладае натыўнае
    паскарэнне да чалавечага сігналу. Промоўшн заблакаваны, флаг
    VICE_TEXT_MATERIALIZATION_V2 выключаны; з флагам 0 дастаўка
    байт-ідэнтычная. Стан: MATERIALIZATION_V2_STATUS_BY.md.

107. ДАСТАЎЛЕНЫ ВЫНІК Materialization v2 на 100 рэальных лоцы
    (2026-07-26, схема справаздачы v3, identity у файле):
    маршрут спрацаваў на 43 радках — дастаўленыя SVG-байты змяніліся
    на ўсіх 43, пры гэтым дастаўленая МАСКА не змянілася ні на адным
    (0/100 розных mask-дайджэстаў), 100/100 not_worse, нуль
    GCR-рэгрэсій, усе агрэгаты (mean IoU 0.77379, CVaR10, min IoU,
    max_regression, severe_low) бітава тыя ж. Сутнасць выніку: на 21
    радку дастаўляецца fair-праграма з МЕДЫЯНАЙ 8 path-камандаў
    замест 1774 у піксельнай лесвіцы (~200×), на 22 радках суд
    сумленна пакінуў piксельна-дакладную праграму, астатнія 57 ідуць
    ранейшымі маршрутамі. Гэта менавіта тая мэта плана §0: тая ж
    фізіка, але сапраўдная вектарная геаметрыя (дугі/прамыя) замест
    лесвіцы — рэдагавальнасць і shape intent без страты дакладнасці.
    ВАЖНАЯ ЗАСЦЯРОГА: «тыя ж пікселі» — гэта пры эталоннай шчыльнасці;
    перцэптыўная розніца (гладкасць супраць лесвіцы) жыве на
    маштабаванні, а машынны суд да яе сляпы (ledger 105) — таму
    чалавечага доказу гэта НЕ дае. Кошт латэнтнасці сумленна:
    full p95 5577→6687 мс на лоцы (+20%), warm-стадыя нават хутчэй
    (p95 228→199). Промоўшн застаецца заблакаваным: патрэбен новы
    сляпы раўнд пасля source-freeze (§9 M4) і латэнтнасць у бюджэце
    §12. Флаг VICE_TEXT_MATERIALIZATION_V2 па-ранейшаму выключаны
    па змаўчанні.
