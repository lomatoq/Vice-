...
def _verify_attestation(
    attestation: Path, pair_sha256: str, *, origin: Path,
) -> dict:
    try:
    ...
    if schema == "pcdc-external-raster-vector-corpus-attestation/v2":
        from .proposal_source_attestation import validate_source_attestation

        validated = validate_source_attestation(
            attestation, origin, pair_sha256=pair_sha256,
        )
        return {
            "schema": schema,
            "attested_pair_count": int(validated["pair_count"]),
            "attested_payload_file_count": int(
                validated["payload_file_count"]
            ),
            "attested_payload_content_sha256": str(
                validated["payload_content_sha256"]
            ),
            "renderer_prefix": str(validated["renderer_prefix"]),
        }
    ...
    for row in report.get("inputs", []):
        ...
        if attestation_name:
            ...
            attestation = (
                ...
                if attestation_path else origin / str(attestation_name)
            )
            ...
            verified = _verify_attestation(
                attestation, pair_sha256, origin=origin,
            )
        else:
            verified = {}
        ...
        if verified.get("attested_pair_count", len(origin_rows)) != len(origin_rows):
            raise ValueError(f"mixed origin attested pair count mismatch: {origin}")
        if (
            verified.get("attested_payload_file_count", payload_count)
            != payload_count
            or verified.get(
                "attested_payload_content_sha256", payload_sha256,
            ) != payload_sha256
        ):
            raise ValueError(f"mixed origin attested payload mismatch: {origin}")
        if (
            verified.get("renderer_prefix") is not None
            and row.get("renderer_prefix") != verified["renderer_prefix"]
        ):
            raise ValueError(
                f"mixed origin renderer attestation mismatch: {origin}"
            )
    return report


def build_mixed_corpus(
    roots: tuple[Path, ...], out: Path, *,
    attestation_overrides: tuple[Path, ...] = (),
) -> dict:
    if len(roots) < 2:
    ...
    override_by_origin = {}
    for override in attestation_overrides:
        attestation = override.resolve()
        payload = json.loads(attestation.read_text("utf-8"))
        origin = str(Path(str(payload.get("origin_root", ""))).resolve())
        if not origin or origin in override_by_origin:
            raise ValueError("duplicate or malformed attestation override")
        override_by_origin[origin] = attestation
    for root in resolved:
        ...
        attestation = override_by_origin.get(str(root))
        overridden = attestation is not None
        ...
        if attestation.is_file():
            verified = _verify_attestation(
                attestation, pair_sha256, origin=root,
            )
            input_record.update({
            ...
            if overridden:
                input_record["attestation_path"] = str(attestation)
            renderer_prefix = verified.get("renderer_prefix")
            if renderer_prefix is not None:
                annotated = []
                for row in part:
                    augmentation = dict(row.get("augmentation", {}))
                    codec = (
                        "jpeg"
                    ...
                    augmentation["renderer"] = (
                        f"{renderer_prefix}-{codec}/v1"
                    )
                    annotated.append({**row, "augmentation": augmentation})
                part = annotated
        identifiers.update(str(row["id"]) for row in part)
        rows.extend(part)
        inputs.append(input_record)
    unused_overrides = set(override_by_origin) - {str(root) for root in resolved}
    if unused_overrides:
        raise ValueError("attestation override does not belong to an input root")
...
def main() -> None:
    ...
    parser.add_argument(
        "--attestation-override", type=Path, action="append", default=[],
    )
    parser.add_argument("--out", type=Path, required=True)
    arguments = parser.parse_args()
    print(json.dumps(
        build_mixed_corpus(
            tuple(arguments.root), arguments.out,
            attestation_overrides=tuple(arguments.attestation_override),
        ),
