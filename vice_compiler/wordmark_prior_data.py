...
def wordmark_data_recipe() -> dict[str, object]:
    return {
        ...
        "text_length": [TEXT_MINIMUM_LENGTH, TEXT_MAXIMUM_LENGTH],
        "text_modes": [
        ...
        "topology_supervision": [
            "small-component-balanced-support",
            "enclosed-counter-balanced-background",
            "two-sided-boundary-band",
            "true-text-length-slices",
            "ocr-hint-exactness-slices",
        ],
...
def _sample_text(generator: np.random.Generator, maximum: int) -> str:
    length = int(generator.integers(
        TEXT_MINIMUM_LENGTH, min(TEXT_MAXIMUM_LENGTH, maximum) + 1,
    ))
    ...
    if mode == 0:
        ...
        # A logo can be a one-glyph monogram or a two-letter mark (HP/GE/VW).
        # Keep the two-word mode impossible below four visible characters, but
        # never resample those short lengths away from the serving contract.
        alphabet = WORDMARK_CHARACTERS.rstrip(" ")
...
def _corrupt_text_hint(
    ...
    elif event < 0.91 and len(hint) > 1:
        del hint[index]
...
def topology_importance_target(support: np.ndarray) -> np.ndarray:
    """Balance pixels whose loss can change components or enclosed counters.

    Global BCE/Dice is dominated by large stems and the exterior background.
    A missing dot or a filled counter therefore barely changes the old loss
    despite changing the exact SVG topology.  This deterministic map raises
    the contribution of small foreground components, enclosed background
    components and both sides of every boundary, then normalizes the mean so
    the optimizer's global learning-rate scale remains stable.
    """
    mask = np.ascontiguousarray(np.asarray(support, bool))
    if mask.ndim != 2 or not np.any(mask):
        raise ValueError("topology importance requires non-empty 2D support")
    ...
    for label in range(1, count):
        ...
        # A one-pixel dot receives the largest boost; large stems remain close
        # to the base pixel objective.
        boost = min(10.0, 18.0 / float(np.sqrt(area)))
        weight[labels == label] += np.float32(boost)

    inverse = (~mask).astype(np.uint8)
    hole_count, hole_labels, _stats, _centroids = (
    ...
    for label in range(1, hole_count):
        ...
        weight[hole] += 8.0
        ring = cv2.dilate(hole.astype(np.uint8), kernel).astype(bool) & mask
        weight[ring] += 4.0

    weight /= max(1.0e-6, float(np.mean(weight)))
    return np.ascontiguousarray(np.clip(weight, 0.25, 12.0), np.float32)
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        for retry in range(32):
            ...
            return {
                ...
                "topology_weight": torch.from_numpy(
                    topology_importance_target(support),
                )[None],
                "text_tokens": torch.from_numpy(token_ids),
                "text_length": torch.tensor(text_length, dtype=torch.long),
                "true_text_length": torch.tensor(
                    len(text), dtype=torch.long,
                ),
                "ocr_hint_exact": torch.tensor(
                    text_hint == text, dtype=torch.bool,
                ),
