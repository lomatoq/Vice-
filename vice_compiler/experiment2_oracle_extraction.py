...
def _oracle_machine_gate(
    *, acceptable_rate: float, class_floor: float, catastrophes: int,
    median_ms: float, p95_ms: float, fallback_feasible: bool,
) -> bool:
    """Reject a hidden semantic-class collapse despite a good overall mean."""
    return bool(
        acceptable_rate >= 0.98
        and class_floor >= 0.95
        and catastrophes == 0
        and median_ms < 20.0 and p95_ms < 100.0
        and fallback_feasible
    )
...
def build_report(corpus_dir: Path = CORPUS) -> dict[str, Any]:
    ...
    class_floor = min(
        float(row["acceptable_scene_rate"]) for row in by_class.values()
    )
    gate = _oracle_machine_gate(
        acceptable_rate=acceptable_rate, class_floor=class_floor,
        catastrophes=catastrophes, median_ms=median, p95_ms=p95,
