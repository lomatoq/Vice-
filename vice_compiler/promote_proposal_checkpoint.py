...
def promote_candidate(
    ...
    if (
        campaign.get("evaluation_source_sha256")
        != evaluation_source_sha256(
            "vice_compiler/experiment12_full_campaign.py",
        )
        or campaign.get("compiler_fingerprint")
        != _compiler_fingerprint(candidate, evaluation_manifest)
    ):
        raise RuntimeError("Phase-12 campaign evaluation code is stale")
    technical_names = (
