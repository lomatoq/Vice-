...
def relation_supervision(
    ...
    if contract.get("family") != family:
        # Deterministic codec/detail counterfactuals add an auxiliary risk
        # query to the same rendered row.  It has a hard-negative class target,
        # but it must not inherit or negate the base macro's semantic
        # relations.  Every other family mismatch remains a corrupt contract.
