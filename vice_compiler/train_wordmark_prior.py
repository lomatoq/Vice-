...
FULL_TRAINING_MINIMUM_VARIANTS = 2_000_000
REPRESENTATIVE_PILOT_MINIMUM_VARIANTS = 131_072
REPRESENTATIVE_PILOT_MINIMUM_HELD_OUT = 4_096
...
def _training_contract(args: argparse.Namespace) -> dict[str, Any]:
    pilot_path = args.representative_pilot_report
    return {
        ...
        "representative_pilot_report": (
            str(pilot_path.resolve()) if pilot_path is not None else None
        ),
        "representative_pilot_report_sha256": (
            _sha256(pilot_path) if pilot_path is not None else None
        ),
...
def representative_pilot_gate(report: dict[str, Any]) -> dict[str, bool]:
    """Return the frozen scale-up gate that must precede a 2M run."""
    held_out = report.get("held_out_test", {})
    slices = held_out.get("diagnostic_slices", {})
    recipe = report.get("data_recipe", {})

    def metric(slice_name: str, name: str) -> float:
        row = slices.get(slice_name, {})
        return float(row.get(name, 0.0))

    checks = {
        "schema_current": (
            report.get("schema") == "pcdc-wordmark-prior-training/v1"
        ),
        "model_data_contract_current": (
            report.get("model_data_contract_sha256")
            == wordmark_prior_source_sha256()
        ),
        "trainer_contract_current": (
            report.get("trainer_source_sha256")
        ...
        "training_variants_ge_131072": (
            int(report.get("training_variants", 0))
            >= REPRESENTATIVE_PILOT_MINIMUM_VARIANTS
        ),
        "held_out_samples_ge_4096": (
            int(report.get("held_out_samples_per_split", 0))
            >= REPRESENTATIVE_PILOT_MINIMUM_HELD_OUT
        ),
        "support_iou_ge_088": (
            float(held_out.get("decoded_support_iou", 0.0)) >= 0.88
        ),
        "topology_accuracy_ge_090": (
            float(held_out.get("decoded_topology_accuracy", 0.0)) >= 0.90
        ),
        "complex_topology_ge_085": (
            float(
                held_out.get("decoded_complex_topology_accuracy", 0.0)
            ) >= 0.85
        ),
        "component_head_ge_088": (
            float(held_out.get("component_head_accuracy", 0.0)) >= 0.88
        ),
        "hole_head_ge_088": (
            float(held_out.get("hole_head_accuracy", 0.0)) >= 0.88
        ),
        "length1_topology_ge_094": (
            metric("length:1", "decoded_topology_accuracy") >= 0.94
        ),
        "length2_topology_ge_092": (
            metric("length:2", "decoded_topology_accuracy") >= 0.92
        ),
        "long_topology_ge_085": (
            metric("length:17-32", "decoded_topology_accuracy") >= 0.85
        ),
        "long_component_head_ge_080": (
            metric("length:17-32", "component_head_accuracy") >= 0.80
        ),
        "long_hole_head_ge_080": (
            metric("length:17-32", "hole_head_accuracy") >= 0.80
        ),
    }
    return {"passed": all(checks.values()), **checks}


def _validate_representative_pilot(path: Path) -> dict[str, Any]:
    report = json.loads(path.read_text("utf-8"))
    checks = representative_pilot_gate(report)
    if not checks["passed"]:
        failed = sorted(name for name, value in checks.items() if not value)
        raise RuntimeError(
            "representative wordmark pilot does not authorize a 2M run: "
            + ", ".join(failed)
        )
    return report
...
def _loss(
    ...
    topology_weight = batch.get("topology_weight")
    if topology_weight is None:
        topology_weight = torch.ones_like(target)
    topology_weight = topology_weight.to(dtype=logits.dtype)
    ...
    bce = torch.sum(bce_map * topology_weight) / torch.clamp(
        torch.sum(topology_weight), min=1.0,
    )
    probability = torch.sigmoid(logits)
    intersection = torch.sum(
        probability * target * topology_weight, dim=(1, 2, 3),
    )
    dice = 1.0 - torch.mean(
        (2.0 * intersection + 1.0)
        / (
            torch.sum(probability * topology_weight, dim=(1, 2, 3))
            + torch.sum(target * topology_weight, dim=(1, 2, 3))
            + 1.0
        )
    )
    sdf_map = F.smooth_l1_loss(
        output["sdf"], batch["sdf"], beta=0.10, reduction="none",
    )
    sdf = torch.sum(sdf_map * topology_weight) / torch.clamp(
        torch.sum(topology_weight), min=1.0,
    )
    component_weight = _count_sample_weight(batch["components"])
    hole_weight = _count_sample_weight(batch["holes"])
    component = _focal_count_loss(
        output["component_logits"], batch["components"],
        sample_weight=component_weight,
    )
    holes = _focal_count_loss(
        output["hole_logits"], batch["holes"], sample_weight=hole_weight,
    )
    ...
    component_regression = _count_regression_loss(
        output["component_count_estimate"], batch["components"],
        sample_weight=component_weight,
    )
    hole_regression = _count_regression_loss(
        output["hole_count_estimate"], batch["holes"],
        sample_weight=hole_weight,
    )
    total = (
        bce + 0.70 * dice + 0.30 * sdf
        + 0.55 * (component + holes)
        + 0.55 * (component_ordinal + hole_ordinal)
        + 0.15 * (component_regression + hole_regression)
    )
    return total, {
        ...
        "component_regression": float(component_regression.detach()),
        "hole_regression": float(hole_regression.detach()),
    }


def _count_sample_weight(target: torch.Tensor) -> torch.Tensor:
    values = target.to(torch.float32)
    return 1.0 + 1.5 * torch.sqrt(torch.clamp(values, min=0.0) / 64.0)


def _weighted_mean(
    values: torch.Tensor, sample_weight: torch.Tensor | None,
) -> torch.Tensor:
...
def _ordinal_count_loss(
    ...
    # ``binary_cross_entropy(probabilities, ...)`` is deliberately rejected by
    # CUDA autocast because a half-precision sigmoid/CDF cannot provide stable
    # gradients.  Preserve the proven FP32 BCE operation exactly, but isolate
    # it from the production AMP region.  An algebraically expanded BCE changed
    # rounding and was enough to destabilize the topology decoder trajectory.
    with torch.autocast(device_type=logits.device.type, enabled=False):
        probability_cdf = torch.clamp(
            torch.cumsum(torch.softmax(logits.float(), dim=1), dim=1),
        ...
        per_sample = F.binary_cross_entropy(
            probability_cdf, target_cdf, reduction="none",
        ).mean(dim=1)
    return _weighted_mean(per_sample, sample_weight)


def _count_regression_loss(
    estimate: torch.Tensor, target: torch.Tensor, *,
    sample_weight: torch.Tensor | None = None,
) -> torch.Tensor:
    per_sample = F.smooth_l1_loss(
        estimate, target.to(estimate.dtype), beta=1.0, reduction="none",
    )
    return _weighted_mean(per_sample, sample_weight)
...
def _length_slice(length: int) -> str:
    if int(length) <= 2:
...
def evaluate(
    ...
    *, thresholds: tuple[float, ...] = (
    ...
    slice_totals: dict[float, dict[str, dict[str, float]]] = {
        threshold: {} for threshold in evaluated_repair_thresholds
    }
    ...
    for batch in loader:
        ...
        true_components = moved["components"].cpu().numpy()
        true_holes = moved["holes"].cpu().numpy()
        true_lengths = moved.get(
            "true_text_length", moved["text_length"],
        ).cpu().numpy()
        hint_exact = moved.get("ocr_hint_exact")
        hint_exact_values = (
            hint_exact.cpu().numpy()
            if hint_exact is not None else np.ones(len(target), bool)
        )
        for index in range(len(target)):
            predicted = (
            ...
            for repair_threshold in evaluated_repair_thresholds:
                ...
                slice_names = (
                    f"length:{_length_slice(int(true_lengths[index]))}",
                    f"ocr_hint_exact:{str(bool(hint_exact_values[index])).lower()}",
                )
                for slice_name in slice_names:
                    sliced = slice_totals[repair_threshold].setdefault(
                        slice_name,
                        {
                            "samples": 0.0, "iou": 0.0,
                            "component": 0.0, "hole": 0.0, "joint": 0.0,
                            "topology": 0.0, "complex_samples": 0.0,
                            "complex_topology": 0.0,
                        },
                    )
                    sliced["samples"] += 1.0
                    sliced["iou"] += _iou(target[index], selected)
                    sliced["component"] += float(predicted[0] == truth[0])
                    sliced["hole"] += float(predicted[1] == truth[1])
                    sliced["joint"] += float(predicted == truth)
                    sliced["topology"] += float(signature == truth)
                    sliced["complex_samples"] += float(complex_sample)
                    sliced["complex_topology"] += float(
                        complex_sample and signature == truth
                    )
    ...
    for name, totals in sorted(
        slice_totals[selected_repair_threshold].items(),
    ):
        slice_samples = max(1.0, totals["samples"])
        complex_samples = max(1.0, totals["complex_samples"])
        selected_slices[name] = {
            "samples": int(totals["samples"]),
            "decoded_support_iou": totals["iou"] / slice_samples,
            "component_head_accuracy": totals["component"] / slice_samples,
            "hole_head_accuracy": totals["hole"] / slice_samples,
            "joint_topology_head_accuracy": totals["joint"] / slice_samples,
            "decoded_topology_accuracy": totals["topology"] / slice_samples,
            "decoded_complex_topology_accuracy": (
                totals["complex_topology"] / complex_samples
            ),
        }
    return {
        "samples": samples,
        ...
        **decoded_metrics,
        ...
        "diagnostic_slices": selected_slices,
    }
...
def train(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if int(args.training_variants) >= FULL_TRAINING_MINIMUM_VARIANTS:
        if args.representative_pilot_report is None:
            raise RuntimeError(
                "a 2M wordmark run requires a passing representative pilot",
            )
        _validate_representative_pilot(args.representative_pilot_report)
    ...
    report["representative_pilot_gate"] = representative_pilot_gate(report)
    return report
...
def main() -> None:
    ...
    parser.add_argument("--representative-pilot-report", type=Path)
