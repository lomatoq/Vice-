...
from .appearance_macros import (
    AppearanceFitRecord,
    oklab_alpha_to_linear_premultiplied,
)
...
def _free_curve_element(
    ...
    if (
        not fitted
        and candidate.certificates.components > 1
        and "refined_source_id" not in dict(candidate.program.parameters)
    ):
        # A single logical logomark may contain several disconnected islands.
        # It has no one closed spline to refine, so retain the proof-checked
        # compound contour rather than silently rejecting it or pretending it
        # is a text line.  This fallback is limited to compound topology and
        # must beat the same strict native-lattice delivery wall.
        component_count, labels = cv2.connectedComponents(
            np.asarray(visible, np.uint8), connectivity=8,
        )
        component_paths = []
        for label in range(1, component_count):
            component_mask = labels == label
            component_path = (
                _fitted_mask_path(component_mask)
                or _mask_path(component_mask)
            )
            if not component_path:
                component_paths = []
                break
            component_paths.append(component_path)
        compound = " ".join(component_paths)
        if compound:
            proof_svg = (
                f'<svg xmlns="http://www.w3.org/2000/svg" '
                f'width="{reir.width}" height="{reir.height}" '
                f'viewBox="0 0 {reir.width} {reir.height}">'
                f'<path d="{compound}" fill="#000" fill-rule="evenodd"/>'
                "</svg>"
            )
            rendered = render_svg_roundtrip(
                proof_svg, width=reir.width,
            ...
            if (
                topology_signature(rendered) == topology_signature(visible)
                and intersection / max(1, union) >= 0.92
                and _mask_boundary_f(rendered, visible) >= 0.97
            ):
                fitted = compound
    if not fitted:
        return None
