    ...
    if model.jpeg_quality is not None:
        ...
        with io.BytesIO() as buffer:
            source_image = Image.fromarray(
                np.clip(composite * 255.0 + 0.5, 0, 255).astype(np.uint8),
                "RGB",
            )
            try:
                source_image.save(
                    buffer, format="JPEG", quality=model.jpeg_quality,
                    subsampling=2,
                )
            finally:
                source_image.close()
            ...
            with Image.open(buffer) as encoded:
                with encoded.convert("RGB") as converted:
                    decoded = np.asarray(
                        converted, dtype=np.float32,
                    ).copy() / 255.0
