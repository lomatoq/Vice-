    ...
    for index in range(len(dataset)):
        try:
            ...
            row = {
                ...
                "source": str(rows[index].get("source", "")),
...
def _degradation_families(row: dict) -> set[str]:
    ...
    if augmentation.get("jpeg_quality") is not None:
        result.add("jpeg")
    if augmentation.get("webp_quality") is not None:
        result.add("webp")
...
def audit_training_data(
    ...
    error_problem_counts = Counter(
        problem for row in errors for problem in row["problems"]
    )
    error_source_counts = Counter(str(row.get("source", "")) for row in errors)
    error_split_counts = Counter(str(row.get("split", "")) for row in errors)
    return {
        "schema": AUDIT_SCHEMA,
        ...
        "error_problem_counts": dict(sorted(error_problem_counts.items())),
        "error_source_counts": dict(sorted(error_source_counts.items())),
        "error_split_counts": dict(sorted(error_split_counts.items())),
        "errors": errors[:100],
