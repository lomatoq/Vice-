...
from .explicit_svg_owners import (
    explicit_svg_owners, has_explicit_owner_contract,
)

MINIMUM_SVG_ALIGNMENT_IOU = 0.35
...
def svg_owner_templates(svg: str, *, render_width: int = 512) -> SvgOwnerTemplates:
    if has_explicit_owner_contract(svg):
        explicit = explicit_svg_owners(svg, render_width=render_width)
        unsupported = {
            row.family for row in explicit.owners
            if row.family not in {"text_line", "whole_shape"}
        }
        if unsupported:
            raise ValueError(
                "explicit SVG owner cannot be represented by text/mark templates"
            )
        result = SvgOwnerTemplates(
            explicit.full_mask,
            tuple(row.mask for row in explicit.owners if row.family == "text_line"),
            tuple(row.mask for row in explicit.owners if row.family == "whole_shape"),
        )
        result.validate()
        return result
...
def _shift(mask: np.ndarray, dx: int, dy: int) -> np.ndarray:
    ...
    sx1 = max(0, -dx)
    sx2 = min(width, width - dx)
    sy1 = max(0, -dy)
    sy2 = min(height, height - dy)
...
def augmented_svg_owner_targets(
    ...
    if alignment_iou < MINIMUM_SVG_ALIGNMENT_IOU:
        return (), float(alignment_iou)
    targets: list[tuple[str, np.ndarray]] = []
    projected_text: list[np.ndarray] = []
    for mask in templates.text_masks:
        ...
        if int(np.sum(projected)) >= 3:
            ...
            projected_text.append(projected)
    # A glyph-group query owns the compound glyph set, while TextLine queries
    # own its individual rows.  Duplicating glyph_group once per row made a
    # three-row source require six positive slots and rendered global Recall@5
    # mathematically impossible.  One exact union keeps the semantic contracts
    # distinct and every generated scene within the five-slot promotion metric.
    if projected_text:
        glyph_group = np.logical_or.reduce(projected_text)
        if int(np.sum(glyph_group)) >= 3:
            targets.append(("glyph_group", _freeze(glyph_group)))
