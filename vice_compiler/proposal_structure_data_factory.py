...
from .proposal_text_data_factory import (
    _add_noise, _jpeg_roundtrip, _slug, _webp_roundtrip,
)
...
def _repeat_svg(rng: random.Random) -> tuple[str, str]:
    # Mirror is a distinct relation token, while every mirror pair is also a
    # repeated-instance group under a reflected transform.  Keep enough mirror
    # formations for >=100 positive held-out labels without deleting the
    # non-mirror grid/radial/row formations.
    prototype = rng.choices(
        ("grid", "radial", "row", "mirror"),
        weights=(1, 1, 1, 9), k=1,
    )[0]
...
def _relation_contract(family: str, prototype: str) -> dict:
    if family == "stroke_network":
        ...
        positive = ("same_group", "repeat", "mirror")
...
def _source_split(source_id: str) -> str:
    group = f"typed-structure-source:{source_id}"
...
def build_pair(source: dict, variant: int, out: Path, *, seed: int = SEED) -> dict:
    ...
    webp_quality = None
    if _source_split(str(source["id"])) == "test" and int(variant) == 1:
        # A renderer+codec formation absent from train is a real untouched
        # observation holdout, not a post-hoc split label.
        jpeg_quality = None
        webp_quality = rng.choice((82, 74, 66))
    ...
    if jpeg_quality is not None:
        ...
        canvas = _webp_roundtrip(canvas, webp_quality)
    ...
    return {
        ...
        "augmentation": {
            ...
            "renderer": (
                ...
                if webp_quality is not None else
                "resvg-pillow-jpeg/v1"
                if jpeg_quality is not None else
                "resvg-pillow-png/v1"
            ),
        },
    }
