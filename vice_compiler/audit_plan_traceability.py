...
def _experiment_evidence(
    ...
    checks = {
        ...
        "evaluation_hash_current": bool(
            payload and payload.get("evaluation_source_sha256")
            == current_evaluator
        ),
    ...
    return {
        ...
        "actual_evaluation_source_sha256": (
            payload.get("evaluation_source_sha256") if payload else None
        ),
...
def _experiment(relative: str, schema: str, *, source_relative: str) -> bool:
    return bool(_experiment_evidence(
        relative, (schema,), source_relative=source_relative,
    )["complete"])
...
def _glyph_prior_training_evidence() -> dict[str, Any]:
    ...
    current_model_contract = glyph_prior_source_sha256()
    reported_model_contract = (
        report.get("contract", {}).get("glyph_prior_source_sha256")
        if report else None
    )
    compatibility = glyph_prior_contract_compatibility(
        reported_model_contract
    )
    checks = {
        ...
        "model_data_contract_current": bool(
            report and current_model_contract and compatibility is not None
        ),
    ...
    return {
        ...
        "reported_model_contract_sha256": reported_model_contract,
        "model_contract_compatibility": compatibility,
...
def _wordmark_prior_training_evidence() -> dict[str, Any]:
    ...
    short_logo = _read_json(
        "benchmarks/pcdc_pre_v14/wordmark_short_logo_audit.json"
    )
    checkpoint_sha256 = (
        hashlib.sha256(checkpoint_path.read_bytes()).hexdigest()
        if checkpoint_path.is_file() else None
    ...
    try:
        ...
        current_trainer_contract = wordmark_trainer_source_sha256()
        current_short_audit_contract = short_logo_audit_source_sha256()
    except (ImportError, OSError, RuntimeError):
        current_model_contract = ""
        current_trainer_contract = ""
        current_short_audit_contract = ""
        SHORT_LOGO_LENGTHS = (1, 2)

        def short_logo_gate(_slices: dict[str, Any]) -> bool:
            return False
    held_out = report.get("held_out_test", {}) if report else {}
    recipe = report.get("data_recipe", {}) if report else {}
    training_contract = report.get("training_contract", {}) if report else {}
    checks = {
        "report_exists": report_path.is_file() and report is not None,
        "checkpoint_exists": checkpoint_path.is_file(),
        "schema_matches": bool(
        ...
        "trainer_contract_current": bool(
            report and current_trainer_contract
            and report.get("trainer_source_sha256")
            == current_trainer_contract
        ),
        "full_serving_recipe_v5": bool(
            recipe.get("schema") == "pcdc-wordmark-procedural-data/v9"
            and recipe.get("text_length") == [1, 32]
        ),
        "deterministic_training_contract": bool(
            training_contract.get("deterministic_algorithms") is True
        ...
        "one_two_symbol_audit_current_and_passed": bool(
            report and short_logo_path.is_file() and short_logo
            and short_logo.get("schema")
            == "pcdc-wordmark-short-logo-audit/v1"
            and short_logo.get("gate_pass") is True
            and short_logo.get("status") == "passed"
            and short_logo.get("audit_source_sha256")
            == current_short_audit_contract
            and short_logo.get("checkpoint_sha256") == checkpoint_sha256
            and short_logo.get("training_report_sha256")
            == hashlib.sha256(report_path.read_bytes()).hexdigest()
            and short_logo.get("model_data_contract_sha256")
            == current_model_contract
            and short_logo.get("trainer_source_sha256")
            == current_trainer_contract
            and short_logo.get("lengths") == list(SHORT_LOGO_LENGTHS)
            and short_logo_gate(short_logo.get("slices", {}))
        ),
    }
    return {
        "complete": all(checks.values()), "checks": checks,
        "report": str(report_path.relative_to(PROJECT)),
        "short_logo_audit": str(short_logo_path.relative_to(PROJECT)),
        "checkpoint": str(checkpoint_path.relative_to(PROJECT)),
        "checkpoint_sha256": checkpoint_sha256,
        "current_model_contract_sha256": current_model_contract,
        "current_trainer_contract_sha256": current_trainer_contract,
    }
...
def _current_phase4_ablation_reports() -> tuple[str, str]:
    ...
    for path in directory.glob("*.json") if directory.is_dir() else ():
        ...
        if (
            ...
            or payload.get("evaluation_source_sha256") != current_evaluator
    ...
    *, expected_wordmark_sha256: str | None = None,
    ...
    checks = {
        "candidate_report_current": bool(
            candidate
            and candidate.get("compiler_source_sha256") == current_hash
            and candidate.get("evaluation_source_sha256") == current_evaluator
        ),
        "baseline_report_current": bool(
            baseline
            and baseline.get("compiler_source_sha256") == current_hash
            and baseline.get("evaluation_source_sha256") == current_evaluator
        ),
...
def build_report() -> dict:
    ...
    phase4_evidence = _experiment_evidence(
        ...
        source_relative="vice_compiler/experiment4_textline.py",
    )
    phase5_evidence = _experiment_evidence(
        "benchmarks/pcdc_experiment5/report.json", ("pcdc-experiment5/v1",),
        source_relative="vice_compiler/experiment5_complexity_stress.py",
    )
    ...
    phases = {
        ...
        "phase4_text": [
            ...
            _row(
                ...
                and _source_contract("vice_compiler/audit_wordmark_short_logo.py", (
                    "FixedLengthWordmarkDataset", "evaluate_symbol_slices",
                    "short_logo_gate",
                ))
                and _source_contract("vice_compiler/text_macros.py", (
            ...
            _row(
                "a one-symbol logo is not forced into the text lane",
                _behavior_contract(
                    "vice_compiler/shape_macros.py",
                    ("free_curve", "MacroKind.SHAPE", "phase5-whole-shape",
                     "topology_signature"),
                    "test_pcdc_phase5.py",
                    ("test_one_compound_logomark_is_a_whole_shape_not_forced_text",
                     "test_one_ambiguous_symbol_keeps_text_and_shape_hypotheses",
                     "Shape/free_curve", "RuntimeMacroCourt",
                     "unsupported_delivery"),
                ),
                "single-symbol glyph/text, typed-shape and arbitrary compound "
                "interpretations remain competing hypotheses",
            ),
            _row(
    ...
    return {
        ...
        "audit_rule": (
            ...
            "measured gates additionally require current compiler/native/"
            "evaluator hashes"
        ),
        "diagnostics": {
