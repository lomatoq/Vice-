...
def _certificate_machine_gate(
    *, correct_rate: float, type_floor: float, micro_recall: float,
    catastrophe_rate: float, ece: float,
) -> bool:
    """Require the court to discriminate every adversarial pair family."""
    return bool(
        correct_rate >= 0.95
        and type_floor >= 0.90
        and micro_recall >= 0.97
        and catastrophe_rate <= 0.005
        and ece <= ECE_TARGET
    )
...
def build_report() -> dict[str, Any]:
    ...
    type_floor = min(
        float(row["correct_choice_rate"]) for row in by_type.values()
    )
    machine_gate = _certificate_machine_gate(
        correct_rate=correct_rate, type_floor=type_floor,
        micro_recall=micro_recall, catastrophe_rate=catastrophe_rate, ece=ece,
    )
    human_gate = bool(
...
def main() -> int:
    ...
    report = bind_report(build_report(), evaluator_source=__file__)
