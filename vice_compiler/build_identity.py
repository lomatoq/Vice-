...
def evaluation_source_sha256(entry: str | Path) -> str:
    """Hash the local Python closure that defines an evaluation artifact.

    Compiler identity proves which SVG implementation was measured. It does
    not change when only a metric, threshold, or experiment harness changes,
    so reports need this independent identity too.
    """
    root = Path(entry)
    ...
    if project not in root.parents or not root.is_file():
        raise ValueError(f"evaluation source is outside the project: {root}")
    pending = [root]
    ...
    while pending:
        ...
        if path in visited:
            continue
        visited.add(path); paths.append(path)
        tree = ast.parse(path.read_text("utf-8"), filename=str(path))
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.ImportFrom):
                if node.level > 0:
                    ...
                    if node.module:
                        candidates.append(
                            base / (node.module.split(".", 1)[0] + ".py")
                        )
                    else:
                        candidates.extend(
                            base / (alias.name.split(".", 1)[0] + ".py")
                            for alias in node.names
                        )
                elif node.module:
                    module = node.module.split(".")
                    candidates.extend((
                        PROJECT.joinpath(*module).with_suffix(".py"),
                        PROJECT.joinpath(*module, "__init__.py"),
                    ))
            ...
            for candidate in candidates:
                ...
                if (
                    resolved.is_file()
                    and project in resolved.parents
                    and resolved not in visited
                ):
                    pending.append(resolved)
    digest = hashlib.sha256(b"pcdc-evaluation-source-identity/v1\0")
...
def bind_report(
    report: dict, *, evaluator_source: str | Path | None = None,
) -> dict:
    ...
    if evaluator_source is not None:
        report["evaluation_source_sha256"] = evaluation_source_sha256(
            evaluator_source,
        )
    return report
