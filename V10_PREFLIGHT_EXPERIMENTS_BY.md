...
### Запіс P-1 — вынік пілота (2026-07-24, адзін прагон, без цюнінгу)

Артэфакт: `benchmarks/pcdc_pre_v14/v10_operator_pilot_report.json`
(16k train / 2k val на held-out сем'ях, 4 эпохі, RTX 4070).

**Факт 1 (пазітыў):** аператары чытаюцца з degraded-растру — усе тры
галавы б'юць majority-baseline: tracking 0.7005 супраць 0.4485 (+25 пт),
effect 0.8315 супраць 0.707, stroke 0.509 супраць 0.3905.

**Факт 2 (негатыў па загадзя запісаным stop):** top-2 прадказаных
варыянтаў губляе 9.2 пт Recall@8 супраць поўнага перабору
(0.696 супраць 0.789 пры mode-baseline 0.557) — больш за дазволеныя
5 пт. Прадказанне вяртае толькі ~60% карысці перабору пры сціску ×4.5.
**Заяўка «top-2 дастаткова» — АДХІЛЕНА і паркуецца.**

**Строгі вынік:** сігнал ёсць, кампрэсія ў такой форме — не.
Наступная законная пастаноўка (новая карта, не рэтрай): крывая
Recall(top-M) супраць бюджэту M — колькі варыянтаў сапраўды трэба
прапаноўваць пры прадказаных размеркаваннях; плюс уваход мадэлі багацейшы
за raster (REIR-каналы). Гэты пілот НІЧОГА не аўтарызуе; поўны трэнінг
v10 па-ранейшаму патрабуе ўласнага readiness-канвеера.

## Experiment G — крывая сем'яў (СТАТУС: ЗАКРЫТЫ, НЕГАТЫЎ на retrieval-узроўні)

Артэфакт: `benchmarks/pcdc_pre_v14/experiment_g_family_curve.json`.
Банк пашыраны да поўнага легальнага рэпазіторыя на запіненай рэвізіі
(§11.2 літаральна): 2001 сям'я / 3726 твараў / 3.9 ГБ; маніфест v2 і
дэскрыптары (3009 твараў) ляжаць ПОБАЧ з атэставаным банкам, які не
кранаецца.

| банк | сем'яў | твараў | unseen R@8 (composed) | R@1 |
|---|---|---|---|---|
| attested | 81 | 241 | **0.8006** | 0.595 |
| v2-600 | 600 | 947 | 0.7946 | 0.622 |
| v2-full | 1987 | 3009 | 0.7946 | 0.622 |

**Факт:** крывая плоская — пашырэнне банка ×24 не падымае тапалагічны
Recall на unseen-шрыфтах. Гіпотэза D5 аўдыту («81 сям'і мала») на
УЗРОЎНІ ТАПАЛАГІЧНАГА ПОШУКУ сфальсіфікавана: тапалагічныя класы гліфаў
пакрыты ўжо малым банкам (сумяшчальна з unseen≈openbank ва ўсіх ранейшых
вымярэннях). Звязваючая вось — кампазіцыя (аператары §8.11) і грубасць
варыянтнай сеткі, не колькасць сем'яў.

**Што застаецца адкрытым сумленна:** D5 можа звязваць (а) геаметрыю
фіту/IoU набліжанай лініі і (б) генералізацыю ТРЭНІРАВАНЫХ мадэляў
(training-side G аўдыту: вучыць на N сем'ях, мераць unseen) — гэта
наступная законная пастаноўка; readiness-гейт `family_learning_curve`
застаецца зачыненым да яе, як і належыць fail-closed.

### Дадатак G-2 — training-side крывая таксама пляскатая (2026-07-24)

Тры пілоты аператарнай CNN (адзін пратакол, val на атэставаных held-out
сем'ях, трэнінг з банка v2 без гэтых сем'яў):
`g_train_fam{81,600,full}.json`.

| сем'яў у трэнінгу | stroke | tracking | effect | сярэдняя |
|---|---|---|---|---|
| 81 | 0.4965 | 0.7040 | 0.8355 | 0.6787 |
| 600 | 0.4580 | 0.7175 | 0.8385 | 0.6713 |
| 1985 | 0.5170 | 0.7105 | 0.8430 | 0.6902 |

+1.2 пт ад ×24 сем'яў, з правалам пасярэдзіне — пляскатасць на
АПЕРАТАРНЫМ зандажы (што і чакана: строук/трэкінг/эфект — стыле-
агнастычныя геаметрычныя ўласцівасці). Вердыкт D5 удакладнены: для
аператараў і тапалагічнага пошуку разнастайнасць сем'яў НЕ звязвае;
адзіны зандаж, дзе яна яшчэ можа звязваць — форма гліфаў (Stage A).
Гейт `family_learning_curve` чакае менавіта яго.

### Дадатак G-3 — Stage-A зандаж: КРЫВАЯ РАСЦЕ, D5-трылогія закрыта (2026-07-24)

Артэфакты: `stage_a_probe_fam{81,600,full}.json`
(degraded 64×64 гліф → класы кампанентаў × дзірак, val на атэставаных
held-out сем'ях; трэнер `train_v10_stage_a_probe.py`).

| сем'яў | components | holes | JOINT |
|---|---|---|---|
| 81 | 0.9980 | 0.8430 | 0.8410 |
| 600 | 0.9975 | **0.8850** | **0.8825** |
| 1985 | 0.9980 | 0.8785 | 0.8765 |

**Факт:** на зандажы формы гліфаў крывая РАСЦЕ: +4.2 пт joint ад 81→600
сем'яў, з насычэннем каля ~600. Поўны вердыкт D5 (тры зандажы):
- аператары §8.11 — НЕ звязвае (пляскатасць);
- тапалагічны пошук — НЕ звязвае (пляскатасць);
- **форма гліфаў (дзіркі/каўнтэры) — ЗВЯЗВАЕ, насычэнне ~600 сем'яў.**

Практычная норма для Stage A: банк ~600–1000 сем'яў дастатковы;
далейшая інвестыцыя ў разнастайнасць — у рэндэры/дэградацыі (§11.4),
не ў новыя сем'і.

**Вынік для readiness:** гэта закрыла апошні гейт —
`v10_training_readiness.json` упершыню кажа **TRAIN (8/8)**. Аўтарызаваны
старт праграмы v10 са Stage A; кожны запуск нясе сваю §4.7-дысцыпліну
(tiny overfit → representative pilot → full з learning-curve па сэмплах).
