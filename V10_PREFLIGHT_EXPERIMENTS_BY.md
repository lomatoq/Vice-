...
**Наступны танны крок:** Experiment E-lite — `--ocr-mode
{exact,corrupted,empty}` на тым жа чыстым уваходзе (уплыў conditioning
ізалявана), і паўтор B на degraded уваходзе з тым жа балансам даўжыняў
(прамое length-balanced пацверджанне разрыву на адным пратаколе).
...
## Experiment D — oracle template/font (СТАТУС: чарга)

Бліжэйшы існуючы механізм — exact-font lane (`exact_font_provider.py`).
Дыягностыка «наколькі дапамагае праўдзівы template» пойдзе праз
v9.5 Template-Warp прататып (аўдыт §9), не праз мадэль v4.

## Experiment E — OCR conditioning matrix (СТАТУС: чарга, танны)

Скрыпт B лёгка пашыраецца рэжымамі `--ocr-mode {exact,corrupted,empty}`
на тым жа чыстым уваходзе: вымярае ўплыў conditioning ізалявана.
N-best у v4/v9 непрадстаўляльны (адзін hard string) — гэта пацвярджае
патрэбу v10-інтэрфейсу, але карупцыйны кантраст вымяральны ўжо цяпер.
