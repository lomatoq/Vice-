...
from .build_identity import (
    PRODUCTION_EXTERNAL_MODULES, compiler_source_sha256,
    native_runtime_identity, runtime_model_identity,
)
from .exact_font_provider import SYSTEM_FONT_CATALOG
from .experiment_inputs import (
    certificate_court_input_identity, font_catalog_input_identity,
...
from .runtime_service import (
    DEFAULT_PROPOSAL_CHECKPOINT, QUALITY_BUDGETS,
    _proposal_promotion_manifest, _validate_proposal_promotion,
)
...
def build_freeze() -> dict[str, Any]:
    ...
    files = {
        ...
        "models": [
            ...
            _entry(DEFAULT_GLYPH_PRIOR_CHECKPOINT),
            _entry(DEFAULT_GLYPH_PRIOR_PROMOTION),
        ],
        "ocr_model": [
    ...
    missing = [
        row["path"] for row in _all_entries(files) if not row.get("exists")
    ]
    ...
    payload = {
        ...
        "complete": bool(
            ...
            and proposal_runtime_enabled and glyph_runtime_enabled
        ...
        "promotion_blockers": (
            ...
            *(() if glyph_runtime_enabled else (
                "runtime glyph prior lacks a valid hash-bound promotion manifest",
            )),
            *(() if exact_text_complete else (
                "exact/OCR/glyph-prior TextLine blind court is incomplete",
            )),
