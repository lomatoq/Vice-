...
def wordmark_data_recipe() -> dict[str, object]:
    return {
        "schema": "pcdc-wordmark-procedural-data/v1",
        ...
        "text_modes": [
            ...
            "lowercase-digits", "mixed-latin-digits",
        ],
...
def _sample_text(generator: np.random.Generator, maximum: int) -> str:
    ...
    if mode == 0:
        ...
        alphabet = WORDMARK_CHARACTERS[:62]
...
def _corrupt_text_hint(
    ...
    if event < 0.86:
        ...
        hint.insert(index, str(generator.choice(tuple(alphabet))))
    elif len(hint) > 1:
