...
def _late_neural_glyph_refinements(
    ...
    """Run the optional glyph prior on certified physical OCR lines.
    ...
    the result remains a separate proposal and must pass the same source-IoU,
    edit-fraction, topology, and physical-line gates as the early lane.
    """
    candidates: list[
    ...
    for proposal in proposals:
        ...
        contract = _glyph_prior_topology_contract(recognized)
        if contract is None:
            continue
        glyphs, components, counters = contract
        expected = (int(sum(components)), int(sum(counters)))
