...
def _typed_reviewed_loci(
    ...
    """Separate model-facing labels from Phase-0 sampling buckets."""
...
def _prepare(
    ...
    relations = np.zeros(8, np.float32)
    relation_mask = np.zeros(8, np.float32)
    text_membership = str(review.get("text_line_membership", "")).lower()
    if text_membership in {"yes", "no"}:
        relation_mask[6] = 1.0
    if text_membership == "yes":
        relations[6] = 1.0
    return {
...
def _counterfactual_rows(
    ...
    for name in HARD_NEGATIVE_TYPES:
        for positive, negative in buckets[name]:
            positives.append(positive); negatives.append(negative); labels.append(name)
    if not positives:
        return (
...
def build_report(
    *, epochs: int = 8, checkpoint: Path = DEFAULT_CHECKPOINT,
    ...
    torch.manual_seed(20260721); np.random.seed(20260721); random.seed(20260721)
    ...
    for epoch in range(max(1, int(epochs))):
        ...
        if selection_key > best_selection_key:
            best_selection_key = selection_key; best_epoch = epoch + 1
            best_state = {
                name: value.detach().cpu().clone()
                for name, value in model.state_dict().items()
            }
    ...
    cache = EvidenceCache()
    hybrid = {}; support_recall = {}
    for name in ("calibration", "test"):
        rows, coverage = _hybrid_entries(prepared[name], neural, cache)
        hybrid.update(rows); support_recall.update(coverage)
    ...
    if len(train_positive):
        ...
        for _ in range(160):
            ...
            ranking_loss.backward(); rank_optimizer.step()
