...
class RasterEvidenceIR:
    ...
    def validate(self) -> None:
        ...
        self.hierarchy.validate(); self.cells.validate(); self.interfaces.validate()
...
def build_boundary_pyramid(raster: CanonicalRaster) -> tuple[BoundaryScale, ...]:
    ...
    for sigma, divisor in levels:
        ...
        center = magnitude
        left = np.roll(center, 1, axis=1); right = np.roll(center, -1, axis=1)
        up = np.roll(center, 1, axis=0); down = np.roll(center, -1, axis=0)
...
def _proposal_tokens(
    ...
    if len(components) >= 2:
        xs = [c[0] for c in components]; ys = [c[1] for c in components]
        x2 = [c[0] + c[2] for c in components]; y2 = [c[1] + c[3] for c in components]
    ...
    l = light.copy(); alpha = raster.straight_rgba[..., 3]
    valid = alpha > 0.05
    if np.any(valid):
        horizontal = float(1.0 - np.mean(np.abs(l - np.fliplr(l))))
        vertical = float(1.0 - np.mean(np.abs(l - np.flipud(l))))
...
def _attach_token_support(
    ...
    for token in tokens:
        ...
        x1 = max(0, min(width, int(x1))); x2 = max(x1, min(width, int(x2)))
        y1 = max(0, min(height, int(y1))); y2 = max(y1, min(height, int(y2)))
...
def _config_fingerprint(max_dim: int, region_size: int) -> str:
    return hashlib.sha256(
        json.dumps(
            {"schema": SCHEMA, "max_dim": max_dim, "region_size": region_size},
            sort_keys=True,
        ).encode("utf-8")
    ).hexdigest()
