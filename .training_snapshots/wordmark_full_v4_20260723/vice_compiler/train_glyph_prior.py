...
from .glyph_prior import (
    GLYPH_CHARACTERS, GLYPH_CHARACTER_SHA256, GlyphPriorConfig, GlyphPriorNet,
    checkpoint_payload, glyph_prior_source_sha256, topology_constrained_support,
    topology_signature,
)
from .glyph_prior_data import (
    TOPOLOGY_ENRICHED_CHARACTERS, OpenFontGlyphDataset, load_font_records,
    sample_digest, split_font_families,
)
...
def run_preflight(args: argparse.Namespace) -> dict:
    ...
    first = dataset[0]; repeated = dataset[0]
...
def train(args: argparse.Namespace) -> dict:
    ...
    for epoch in range(start_epoch + 1, start_epoch + args.epochs + 1):
        ...
        for batch_index, batch in enumerate(loaders["train"], start=1):
            ...
            optimizer.step()
            running += float(loss) * len(characters); samples += len(characters)
