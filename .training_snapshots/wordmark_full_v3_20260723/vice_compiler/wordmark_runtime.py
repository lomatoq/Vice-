...
from .wordmark_prior import (
    WORDMARK_VOCAB_SHA256, WordmarkPriorConfig, WordmarkPriorNet,
    decode_wordmark_support, topology_signature, wordmark_prior_source_sha256,
    wordmark_token_ids,
)
...
def _promotion_evidence_paths_cached(
    ...
    return tuple(
        ...
        for key in (
            "training_report", "experiment4_report", "full_tests_report",
        )
        if payload.get(key)
    )
...
def resolve_wordmark_prior_checkpoint(path: Path | None = None) -> Path | None:
    ...
    if candidate is None:
        candidate = Path(override) if override else DEFAULT_WORDMARK_PRIOR_CHECKPOINT
...
def propose_wordmark_masks(
    ...
    prepared = tuple(_prepare(row, model.config) for row in rows)
    valid_indices = tuple(
