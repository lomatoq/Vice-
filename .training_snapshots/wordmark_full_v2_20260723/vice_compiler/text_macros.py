...
def _late_neural_glyph_refinements(
    ...
    line.  Keep this pass bounded to the best exact-topology line per OCR text.
    The whole-line lane is batched and never creates character-cell seams; both
    neural results remain separate proposals and must pass source-IoU,
    edit-fraction, topology, and physical-line gates.
    """
    candidates: list[
    ...
    for proposal in proposals:
        ...
        if glyph_contract is None and wordmark_contract is None:
            continue
        if glyph_contract is not None:
            ...
            # The whole-line head predicts joined/global topology itself.
            # Punctuation has no sound independent per-cell component law, so
            # proposal score is the deterministic selector for this OCR text.
            topology_distance = 0
        candidates.append((
    ...
    # At most one inference attempt and one emitted proposal per OCR string.
    # Exact topology sorts first, making this a deterministic cheap->expensive
    # cascade instead of multiplying inference by every threshold draft.
    ...
    for _distance, _negative_score, _line_id, recognized, proposal in sorted(
        ...
        if recognized in attempted:
            continue
        attempted.add(recognized)
    ...
    model_root = Path(__file__).resolve().parents[1] / "models"
    wordmark_enabled = bool(wordmark_override) or (
    ...
    if wordmark_enabled:
        ...
        wordmark_results = tuple(None for _ in selected)

    refinements: list[TextLineProposal] = []
    for row, wordmark in zip(selected, wordmark_results):
...
def propose_text_lines(
    ...
    for token in text_tokens:
        ...
        classical_single_glyph = bool(
            token.provenance == "adaptive-foreground-topology-consensus"
            and int(token_parameters.get("topology_components", -1)) == 1
            and (
                consensus_holes >= 1
    ...
    for line in proposals:
        ...
        if (
            ...
            custom = _macro_record(
                ...
                dual=(), prototypes=(),
                parameters=(("glyphs", 1), ("polarity", line.polarity),
                            ("geometry_state", "deferred-after-local-court")),
...
def select_text_line_with_court(
    ...
    for record, mask in unique.values():
        ...
        single_glyph_inverse_recovery = bool(
            ...
            and "token-polarity-reconciled-to-consensus-topology" in line.sources
            and line.score >= 0.85
