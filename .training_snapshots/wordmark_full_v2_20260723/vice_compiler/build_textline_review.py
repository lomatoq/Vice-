...
from .experiment4_textline import (
    CORPUS, TextLineReviewArtifacts, _glyph_prior_identity,
    _wordmark_prior_identity, evaluate_locus,
)
...
def build(*, exact: bool = False) -> dict[str, Any]:
    ...
    payload = {
        ...
        "candidate_mode": (
            "exact-ocr-glyph-and-wordmark-priors"
            if exact else "warm-font-free"
        ),
        "glyph_prior_checkpoint": _glyph_prior_identity() if exact else None,
        "wordmark_prior_checkpoint": (
            _wordmark_prior_identity() if exact else None
        ),
