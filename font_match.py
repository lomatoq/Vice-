...
def _font_has_text(font_path: str | Path, text: str) -> bool:
    ...
    try:
        ...
        present = codepoints.issubset(cmap)
        font.close()
        return present
    except Exception:
    ...
    *,
    ...
    def evaluate(params: dict[str, float]) -> tuple[MaskMetrics, np.ndarray]:
        ...
        if tracking not in rendered:
            rendered[tracking] = render_tracked_text(path, text, tracking)
    ...
    for tracking in tracking_grid:
        try:
            rendered[round(float(tracking), 6)] = render_tracked_text(path, text, tracking)
